...
I've used a bunch of solarized themes. The best I have found is gruvbox. Despite it
...
After installing colorbuddy and gruvbox.nvim with your favorite package manager add
...
gruvbox aims to be simple to customize.

The following is how I setup gruvbox. Ihis changes the color of the WarningMsg
(currently used by some code actions) to a less visible color by linking the group to a
predefined group.

```
lua << EOF
  n = require('gruvbox').setup({
    comment_italics = true,
    background_set = true,
  })
